<?php
//tous les fonctions pour travail avec le tablo "etudiants"
...
function getEtudiant(){
	...
	$requete = "	SELECT * FROM  `etudiants` INNER JOIN `Utilisateurs` ON etudiants.user_id = Utilisateurs.id ";
	$retour = [];
...
function updateEtudiant($id,$nom,$prenom,$group,$email){
	...
	$database = "hop3x";
	$conn = new PDO('mysql:host=localhost;dbname='.$database, $usernam, $password);
	$requete ="UPDATE `etudiants` SET `nom` = '".$nom."' ,`prenom` = '".$prenom."' ,`group` = '".$group."' ,`email` = '".$email."' WHERE id=".$id;
...
function deleteEtudiant($id){
	...
	$database = "hop3x";
	$conn = new PDO('mysql:host=localhost;dbname='.$database, $usernam, $password);
	$requete ="DELETE FROM `etudiants` WHERE `user_id` =".$id;
	$statement = $conn->query($requete);	if($statement== TRUE) {
		echo "<h3>Les modifications ont ete prises en compte</h3>";
	}
	else{
