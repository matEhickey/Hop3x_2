...
function getInstanceuniversitaires(){
	$serverName = "localhost";
	$username = "root";
	$password = "abcde";
	$database = "hop3x";
	...
	$requete = "	SELECT * FROM  `instanceUniversitaire`";
	$retour = [];
...
function isUniversitaire($session_id){
	$enonces = getInstanceuniversitaires();
	foreach($enonces as $enonce){
		if($enonce["session_id"] == $session_id){
			return($enonce["sessionUniv_id"]);
		}
	}
	return(false);
}
